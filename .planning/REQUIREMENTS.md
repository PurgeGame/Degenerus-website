...
- [x] **PROFILE-01**: User can see selected player's current activity score with decomposition (quest streak, mint count, affiliate bonus, deity/whale flags) (Phase 51)
- [x] **PROFILE-02**: User can see selected player's current quest slots with progress, target, and completion state (Phase 51 -- high-difficulty clause struck per D-20)
- [x] **PROFILE-03**: User can see selected player's quest streak counter and last completed day (Phase 51)
- [x] **PROFILE-04**: Profile panels re-render when day scrubber changes effective day (Phase 51)
- [x] **PROFILE-05**: User can see selected player's Daily Activity counts for the selected day: lootboxes purchased, lootboxes opened, tickets purchased, ticket wins (Phase 51)

### TICKETS -- 4-trait quadrant inventory

- [x] **TICKETS-01**: User can see selected player's ticket inventory rendered as 4-trait quadrant cards (Phase 52)
- [x] **TICKETS-02**: Each ticket card visually shows its 4 traits in the 4 quadrants in a consistent aesthetic (Phase 52)
- [x] **TICKETS-03**: Ticket rendering groups 4 consecutive entries per card; partial groups render as pending (Phase 52)
- [x] **TICKETS-04**: Untraited entries (awaiting VRF) render as "pending pack" placeholders, not empty cards (Phase 52)

### PACKS -- Openable reveal experience

- [x] **PACKS-01**: User sees an animated "pack" appear when selected player purchases tickets (Phase 52)
- [x] **PACKS-02**: User sees an animated "pack" appear when selected player wins tickets (from jackpot draws) (Phase 52)
- [x] **PACKS-03**: Pack stays closed until RNG resolves for that entry; opens to reveal 4-trait tickets when traits are assigned (Phase 52)
- [x] **PACKS-04**: Lootbox-sourced tickets open immediately on lootbox open (no RNG wait -- traits are known at lootbox open time) (Phase 52)
- [x] **PACKS-05**: Pack-opening animation uses GSAP timeline with optional sound cue (Phase 52)
...
- [x] **COINFLIP-01**: User can see selected player's coinflip state (deposited amount, claimable preview, auto-rebuy settings) (Phase 54)
- [x] **COINFLIP-02**: User can see the daily coinflip leaderboard with ranks (Phase 54)
- [x] **COINFLIP-03**: User can see current bounty, biggest-flip-today player and amount (Phase 54)

### BAF -- Leaderboard + player standing

- [x] **BAF-01**: User can see selected player's current BAF score and rank at current level/window (Phase 54)
- [x] **BAF-02**: User can see top-4 BAF leaderboard with prominence-based styling matching beta's baf-panel (Phase 54 -- D-06 gold/silver/bronze prominence)
- [x] **BAF-03**: User can see which level/window the BAF round is for and whether it is open (Phase 54)
...
- [x] **JACKPOT-01**: User can see Roll 1/Roll 2 trait reveal animation for the selected effective day's jackpot (Phase 52)
- [x] **JACKPOT-02**: Jackpot widget reuses beta's existing jackpot-panel Roll components without rewrite (Phase 52 -- D-09 wallet-free utils swap, no fork)
- [x] **JACKPOT-03**: Jackpot panel updates when day scrubber changes effective day (Phase 52)
...
- [x] **INTEG-01**: Ship or confirm `GET /player/{addr}/tickets/by-trait` endpoint for trait-grouped ticket inventory (P0 -- blocks TICKETS/PACKS). Phase 50 kickoff (INTEG-01-SPEC.md) + Phase 52 delivery: shipped database repo via 3 atomic commits, 10/10 vitest green
- [x] **INTEG-02**: Ship or confirm extended `GET /player/:address?day=N` endpoint with `scoreBreakdown`, day-aware `quests[]`, `questStreak`, and `dailyActivity` block (Phase 51 -- shipped database repo via d135605 + dab5adf + 64fe8db, 20/20 vitest green)
...
- [x] **INTEG-05**: Ship or confirm per-player BAF score endpoint (Phase 54 -- shipped database repo via a0d4e69 + 6392541 + 08ef417, 14/14 vitest green)
...
| PROFILE-01 | Phase 51 | Validated (2026-04-24) |
| PROFILE-02 | Phase 51 | Validated (2026-04-24) -- high-difficulty clause struck per D-20 |
| PROFILE-03 | Phase 51 | Validated (2026-04-24) |
| PROFILE-04 | Phase 51 | Validated (2026-04-24) |
| PROFILE-05 | Phase 51 | Validated (2026-04-24) |
| TICKETS-01 | Phase 52 | Validated (2026-04-24) |
| TICKETS-02 | Phase 52 | Validated (2026-04-24) |
| TICKETS-03 | Phase 52 | Validated (2026-04-24) |
| TICKETS-04 | Phase 52 | Validated (2026-04-24) |
| PACKS-01 | Phase 52 | Validated (2026-04-24) |
| PACKS-02 | Phase 52 | Validated (2026-04-24) |
| PACKS-03 | Phase 52 | Validated (2026-04-24) |
| PACKS-04 | Phase 52 | Validated (2026-04-24) |
| PACKS-05 | Phase 52 | Validated (2026-04-24) |
...
| COINFLIP-01 | Phase 54 | Validated (2026-04-24) |
| COINFLIP-02 | Phase 54 | Validated (2026-04-24) |
| COINFLIP-03 | Phase 54 | Validated (2026-04-24) |
| BAF-01 | Phase 54 | Validated (2026-04-24) |
| BAF-02 | Phase 54 | Validated (2026-04-24) -- D-06 gold/silver/bronze prominence |
| BAF-03 | Phase 54 | Validated (2026-04-24) |
...
| JACKPOT-01 | Phase 52 | Validated (2026-04-24) |
| JACKPOT-02 | Phase 52 | Validated (2026-04-24) -- D-09 wallet-free utils swap, no fork |
| JACKPOT-03 | Phase 52 | Validated (2026-04-24) |
...
| INTEG-01 | Phase 50 (kickoff), Phase 52 (gate) | Validated (2026-04-24) -- database a46fdcb + e130547 + 64fe8db; 10/10 vitest |
| INTEG-02 | Phase 51 (gate) | Validated (2026-04-24) -- database d135605 + dab5adf + 64fe8db; 20/20 vitest |
...
| INTEG-05 | Phase 54 (gate) | Validated (2026-04-24) -- database a0d4e69 + 6392541 + 08ef417; 14/14 vitest |
