...
// WalletConnect's browser redirect runs only after its async request validation
// and relay publish. The SDK opens HTTPS wallet links in a new browsing context,
// which mobile browsers can reject once the original tap activation has
// expired. Listen for the SDK's public "request sent" event instead and reuse
// this browsing context; `_self` navigation is not a popup, and the exact
// request remains available if the player needs to tap the CTA again.
const WC_DEEPLINK_CHOICE_KEY = 'WALLETCONNECT_DEEPLINK_CHOICE';
const WC_REQUEST_SENT_EVENT = 'session_request_sent';
const WC_HANDOFF_LISTENER_TIMEOUT_MS = 30_000;
...
const _wcScopedHandoffCounts = new WeakMap();
...
function _walletConnectDeepLinkChoice(provider) {
  ...
  } catch (_e) { /* session metadata fallback below */ }
...
function _openWalletDeepLinkInPlace(href) {
  ...
  try {
    if (typeof win.open === 'function') {
      // Reown uses `_blank` for HTTPS links, which is the blocked-popup path on
      // mobile. Reusing `_self` lets the OS universal-link handler foreground
      // the already-connected wallet without creating a disposable browser tab.
      win.open(href, '_self', 'noreferrer noopener');
      return true;
    }
  } catch (_e) { /* location.assign fallback below */ }
  try {
...
function _adjustScopedHandoff(client, topic, delta) {
  let topics = _wcScopedHandoffCounts.get(client);
  if (!topics) {
    if (delta <= 0) return;
    topics = new Map();
    _wcScopedHandoffCounts.set(client, topics);
  }
  const next = Math.max(0, Number(topics.get(topic) || 0) + delta);
  if (next > 0) topics.set(topic, next);
  else topics.delete(topic);
}

function _hasScopedHandoff(client, topic) {
  return Number(_wcScopedHandoffCounts.get(client)?.get(topic) || 0) > 0;
}

// Cover every WalletConnect approval path, including passes and network
// changes that do not originate in the Decimator panel. A scoped purchase
// handle temporarily takes ownership of its topic so the same request is not
// opened twice and can additionally expose its retry CTA.
function _installWalletConnectAutomaticHandoff(provider) {
  ...
  client.on(WC_REQUEST_SENT_EVENT, (event) => {
    const topic = String(event?.topic || '');
    const method = String(event?.request?.method || '');
    if (!topic || event?.id == null || !WC_APPROVAL_METHODS.has(method)) return;
    if (_hasScopedHandoff(client, topic)) return;
    const currentTopic = String(provider?.session?.topic || '');
    if (currentTopic && currentTopic !== topic) return;
...
/**
 * Arm a one-shot mobile handoff before invoking a WalletConnect signing RPC.
 * Returns null for injected wallets or providers without a SignClient.
 *
 * `open()` intentionally remains usable after the automatic attempt so a
 * second, explicit CTA tap can retry the exact same pending request.
 */
export function createWalletConnectRequestHandoff(provider = getEip1193(), {
  onReady = null,
} = {}) {
  if (provider?.isWalletConnect !== true) return null;
  const client = provider?.signer?.client;
  const sessionTopic = String(provider?.session?.topic || '');
  const choice = _walletConnectDeepLinkChoice(provider);
  if (!client || typeof client.on !== 'function' || !sessionTopic || !choice) return null;
  ...
  _adjustScopedHandoff(client, sessionTopic, 1);
  ...
  const stopListening = () => {
    ...
    if (listenerTimer != null) {
      try { clearTimeout(listenerTimer); } catch (_e) { /* defensive */ }
      listenerTimer = null;
    }
    try { client.off?.(WC_REQUEST_SENT_EVENT, onRequestSent); } catch (_e) { /* defensive */ }
    try { client.removeListener?.(WC_REQUEST_SENT_EVENT, onRequestSent); } catch (_e) { /* defensive */ }
  };

  const handle = {
    walletName: choice.walletName,
    get ready() { return Boolean(requestHref) && !canceled; },
    open() {
      if (canceled || !requestHref) return false;
      return _openWalletDeepLinkInPlace(requestHref);
    },
    cancel() {
      canceled = true;
      requestHref = '';
  ...
  function onRequestSent(event) {
    if (canceled) return;
    if (String(event?.topic || '') !== sessionTopic) return;
    if (event?.request?.method !== 'eth_sendTransaction') return;
    if (event?.id == null) return;
    requestHref = _formatWalletConnectRequestDeepLink(choice.href, event.id, sessionTopic);
    stopListening();
    try { onReady?.(handle); } catch (_e) { /* presentation cannot block handoff */ }
    handle.open();
  }

  client.on(WC_REQUEST_SENT_EVENT, onRequestSent);
  listenerTimer = setTimeout(stopListening, WC_HANDOFF_LISTENER_TIMEOUT_MS);
  listenerTimer?.unref?.();
  return handle;
}
