...
const clickPop = node => node.dispatchEvent({ type: 'click', stopPropagation() {} });
async function revealPops(el) {
  for (let i = 0; i < 4; i++) {
    const all = el.querySelector('.dgn-pop__all');
    if (!all) return;
    clickPop(all);
    await tick();
    if (!el.querySelector('.rvl-dgn-spin-cta')?.hidden) return;
  }
}
...
describe('normalizeSequence', () => {
  ...
  test('lootbox legs → cards: opened splits into tickets + flip; spin keeps reel payload', () => {
    ...
    assert.equal(seq.cards[2].label, 'DEGENERETTE CARD');
    ...
    assert.equal(seq.cards[2].revealedLabel, 'ETH CARD');
  ...
  test('multiple BoxSpin legs remain separate verified presentations in receipt order', () => {
    ...
    assert.ok(seq.cards.every((card) => card.label === 'DEGENERETTE CARD'));
    assert.ok(seq.cards.every((card) => card.rarity === 'rare'),
      'pre-spin card styling cannot leak the currency lane');
    assert.deepEqual(seq.cards.map((card) => card.revealedLabel), ['WWXRP CARD', 'FLIP CARDS'],
  ...
  test('day summary records paid BoxSpins without replaying reels and omits zero results', () => {
    ...
    const spin = settledSpins.find((card) => card.label === 'FLIP DEGENERETTE CARD');
    assert.ok(spin, 'the settled spin remains visible in the day receipt');
    assert.equal(spin.label, 'FLIP DEGENERETTE CARD');
    assert.equal(spin.value, '240 FLIP');
    assert.match(spin.sub, /2 of 3 paid/);
    assert.match(spin.sub, /LUCKBOX #17/);
    assert.equal(settledSpins.some((card) => card.label === 'WWXRP DEGENERETTE CARD'), false,
...
describe('buildBoxSpinBoard', () => {
  ...
  test('preserves every verified reel and keeps a three-spin FLIP payout group-level', () => {
    ...
    assert.equal(board.headline, 'LUCKDEGENERETTE CARD');
  ...
  test('a zero-payout record bounty becomes a direct three-reel FLIP board', () => {
    ...
    assert.equal(board.headline, 'BIGGEST CARD BOUNTY');
    ...
    assert.equal(sequence.spinBoard.headline, 'BIGGEST CARD BOUNTY');
  });
  ...
  test('mystery box currency flips after reel one while a known-FLIP record bounty does not', () => {
    assert.match(
      REVEAL_SRC,
      /if \(!currencyRevealed && progress\.masks\[0\] === 15\)[\s\S]*?#appendBoxSpinCurrencyReveal[\s\S]*?interstitial: true/,
    ...
    assert.match(REVEAL_SRC, /availableCount: currencyRevealed \? board\.rows\.length : 1/,
    ...
    assert.doesNotMatch(REVEAL_SRC, /MORE FLIP CARDS · THEN SURVIVAL/);
    ...
    assert.match(
      REVEAL_SRC,
      /#renderFullSpinStage\(board,\s*\{[\s\S]*?launchFromLootbox:\s*Boolean\(options\.launchFromLootbox\)/,
      'a bonus board can enter through the one-time box-launch transition',
    );
  });
});
...
describe('buildDegeneretteSpinFrames', () => {
  test('randomizes all eight component locks and lands on the verified ticket', () => {
    ...
    assert.match(
      REVEAL_SRC,
      /mountPopBoards\(rendered\.compare, board\.rows/,
...
describe('reveal-overlay element', () => {
  ...
  test('foil summary shows both tickets, Degenerette colors, and the exact bonus', async () => {
    ...
    assert.match(chart.querySelector('.rvl-foil-match__foot').textContent,
      /T5 BONUS6-FACE DEGENERETTE CARD/);
    assert.match(APP_CSS,
  ...
  test('ordinary lootbox rewards reach the roomy receipt with detailed pack art and no heading', async () => {
    ...
    assert.match(REVEAL_SRC,
      ...
      'opening the box starts reel one without a redundant REVEAL CARD gate');
  ...
  test('OPEN ALL aggregates selected BoxSpins behind one play action and retires every box once', async (t) => {
    ...
    openAll.dispatchEvent({ type: 'click', stopPropagation() {} });
    for (let i = 0; i < 20
      && summary.querySelector('.rvl-collect-cta')?.textContent !== 'REVEAL 4 CARDS'; i += 1) {
      await tick();
    }
    ...
    assert.equal(spinGrantCard?.querySelector('.rvl-card-label')?.textContent, 'DEGENERETTE CARDS');
    assert.equal(playAll?.textContent, 'REVEAL 4 CARDS');
    assert.equal(spinZone.hidden, true,
      'the combined grant waits for the deliberate REVEAL CARDS click');
    ...
    assert.equal(spinZone.hidden, false,
      'the one REVEAL CARDS confirmation enters the first selected spin');
    await revealPops(el);
  ...
  test('reduced motion keeps manual popping and one tally across every board', async () => {
    ...
    assert.equal(el.querySelector('.rvl-dgn-speed'), null);
  ...
  test('only the completed payout summary splits real ETH from awarded Luckboxes', async () => {
    for (const lootboxEth of [0n, 5n * 10n ** 15n]) {
      ...
      const el = instantiate(); await tick();
      assert.equal(el.querySelector('.rvl-spin-total'), null);
      assert.equal(el.querySelector('.is-running'), null);
      await revealPops(el);
      const total = el.querySelector('.rvl-spin-total');
      assert.equal(total.classList.contains('rvl-spin-total--eth-split'), lootboxEth > 0n);
      assert.equal(/ETH LUCKBOX/.test(total.textContent), lootboxEth > 0n);
      clickPop(el.querySelector('.rvl-dgn-spin-cta')); await tick();
  ...
  test('motion DAY SUMMARY omits a completed zero BoxSpin without replaying it', async () => {
    ...
    try {
      ...
      assert.equal(summary.querySelectorAll('.rvl-card-label')
        .some((label) => label.textContent === 'WWXRP DEGENERETTE CARD'), false,
  ...
  test('a successful survival flip settles on the green ETH face', async () => {
    ...
    assert.equal(grant.querySelector('.rvl-collect-cta').textContent, 'REVEAL CARD');
    ...
    assert.equal(animatedCoin, null, 'the stale animated coin is detached before the static face appears');
    ...
    assert.match(payoutMeter.textContent, /CARD PAYOUT450 FLIPDOUBLE OR NOTHING · WIN 900 FLIP/);
  ...
  test('a survivor uses its emitted payout instead of a pre-flip estimate', async () => {
    ...
    assert.match(
      ...
      /CARD PAYOUT8,000 FLIPDOUBLE OR NOTHING · WIN 16,000 FLIP/,
  ...
  test('a busted record bounty starts as FLIP and keeps its reel stake in the result', async () => {
    ...
    assert.match(zone.querySelector('.rvl-box-payout-meter').textContent,
      /CARD PAYOUT.*FLIP.*DOUBLE OR NOTHING/s);
    assert.match(zone.querySelector('.rvl-survival').textContent,
      /BUSTED.*1 PAYING CARD · \d[\d,.KM]* FLIP LOST/s);
  ...
  test('pending payout text is identical for a fallback loss and a possible win', async (t) => {
    ...
    assert.doesNotMatch(JSON.stringify(snapshots), /[≈~]|PAYING CARDS/);
  });
  ...
  test('a BoxSpin survival bust shows a plausible reel payout without a loss marker', async () => {
    ...
    assert.match(
      payoutMeter.textContent,
      /CARD PAYOUT[\d,.KM]+ FLIPDOUBLE OR NOTHING · WIN [\d,.KM]+ FLIP/,
    );
    assert.match(
      survival.textContent,
      /BUSTED1 PAYING CARD · [\d,.KM]+ FLIP LOST/,
    );
    assert.doesNotMatch(zone.textContent, /[≈~]/,
      'an approximation marker must never give away the survival loss');
    assert.doesNotMatch(
      zone.textContent,
      /PAYOUT AT RISK|WIN LOCKED|CARD PAYOUT AT RISK|FINAL PAYOUT LOST/,
  ...
  test('a BoxSpin survival bust keeps an explicit pre-survival payout exact', async () => {
    ...
    assert.match(
      ...
      /CARD PAYOUT300 FLIPDOUBLE OR NOTHING · WIN 600 FLIP/,
    );
    assert.match(
      zone.querySelector('.rvl-survival').textContent,
      /BUSTED1 PAYING CARD · 300 FLIP LOST/,
    );
  ...
  test('an all-miss FLIP BoxSpin settles as UNLUCKY without a survival section', async () => {
    ...
    const grant = el.querySelector('[data-bind="rvl-summary"]');
    assert.equal(grant.querySelector('.rvl-card-label').textContent, 'DEGENERETTE CARD');
    grant.querySelector('.rvl-collect-cta')
      .dispatchEvent({ type: 'click', stopPropagation() {} });
    await tick();
    await revealPops(el);
