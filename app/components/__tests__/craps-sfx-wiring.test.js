...
test('craps resolution wires one restrained cue to each visible gameplay beat', () => {
  ...
  assert.match(source, /if \(battleWon \|\| this\.#highRollerWonByViewer\) sfxFanfare\(true\);\s*else if \(last\.terminal === 'goal'\) sfxFanfare\(false\);\s*else if \(last\.terminal === 'bust' && !this\.#viewerBustCheckpointPassed\) sfxNoWin\(\);/s,
    'completion reserves the large fanfare for a battle bounty or a high-roller win and never repeats the bust cue');
});
