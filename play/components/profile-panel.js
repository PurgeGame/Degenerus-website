// play/components/profile-panel.js -- <profile-panel> Custom Element stub (Phase 50)
// Phase 50 delivers only the skeleton + subscription plumbing.
// Hydration logic lands in Phase 51.
...
class ProfilePanel extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.innerHTML = `
      <section data-slot="profile" class="panel">
        ...
        <div data-bind="content" hidden>
          <h2>Profile</h2>
          <p class="text-dim">Panel delivered in Phase 51.</p>
        </div>
      </section>
    `;

    // Phase 50: subscriptions prove day-awareness wiring reaches this panel.
    // Phase 51 replaces the console logs with real fetchers + #showContent().
    this.#unsubs.push(
      subscribe('replay.day',    (day)  => console.log('[profile-panel] replay.day =',    day)),
      subscribe('replay.player', (addr) => console.log('[profile-panel] replay.player =', addr)),
    );
  }
